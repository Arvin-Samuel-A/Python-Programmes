...
def reward_function(params):
    ...
    SPEED_THRESHOLD_1=0.975
    
    SPEED_THRESHOLD_2=0.875
    ...
    if direction_diff > DIRECTION_THRESHOLD or not all_wheels_on_track:
        ...
        if next_point in (straight_waypoints):
            if speed>=SPEED_THRESHOLD_1:
                reward+=max(speed,SPEED_THRESHOLD_1)*10
            else:
                reward-=0.1
        else:
            if speed>=SPEED_THRESHOLD_2:
                reward+=max(speed,SPEED_THRESHOLD_2)*10
            else:
                reward-=0.1

    reward += (1- (distance_from_center/track_width*0.5)**0.4)*10
